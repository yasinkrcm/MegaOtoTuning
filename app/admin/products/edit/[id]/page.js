...
export default function EditProduct() {
  ...
  const categories = ['Aksesuar', 'Parça'];
  
  const fetchProduct = useCallback(async () => {
    try {
      const response = await authenticatedFetch(`/api/products/${params.id}`);
  ...
  }, [authenticatedFetch, params.id]);
  
  useEffect(() => {
    if (!authLoading && user) {
      fetchProduct();
    }
  }, [user, authLoading, fetchProduct]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await authenticatedFetch(`/api/products/${params.id}`, {
        method: 'PUT',
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-pink-500 text-black border-r-transparent"></div>
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                {formData.imageUrl && !imagePreview && (
                  <div className="mb-4">
                    <p className="text-sm font-medium text-gray-700 mb-2">Mevcut Resim:</p>
                ...
                <div className="flex items-center space-x-4">
                  <label className="cursor-pointer bg-pink-50 hover:bg-pink-100 text-pink-700 px-4 py-2 rounded-md border border-pink-300 transition-colors">
                    <span className="text-sm font-medium">Yeni Fotoğraf Seç</span>
                    <input
                      type="file"
                      accept="image/*"
                ...
                {imagePreview && (
                  <div className="mt-4">
                    <p className="text-sm font-medium text-gray-700 mb-2">Yeni Resim Önizleme:</p>
                    <div className="relative w-32 h-32 border border-gray-300 rounded-lg overflow-hidden">
                      <Image
                        src={imagePreview}
